...
data class UserInfoRegistrationDto(
    @field:Schema(description = "Уникальный идентификатор пользователя в системе", example = "user123")
    val id: String,

    @field:Schema(description = "Имя пользователя", example = "john_doe")
    val username: String,

    @field:Schema(description = "Короткий идентификатор для отображения", example = "JD123")
    val shortId: String,
)
...
data class UserCredentialsRegistrationDto(
    ...
    @field:Schema(description = "Электронная почта", example = "user@example.com")
    val email: String,
)
...
data class RegistrationResponseDto(
    ...
    @field:Schema(
        description = "Дата и время регистрации (ISO 8601 формат)",
